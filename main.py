...
def create_log_file(player_index):
    timestamp = datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
    filename = f"{timestamp}-bingo-Spieler{player_index + 1}.txt"
    log_files[player_index] = open(filename, 'w')
    log_files[player_index].write(f"{timestamp} Start des Spiels\n")
    return log_files[player_index]


def log_event(player_index, event):
    timestamp = datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
    log_files[player_index].write(f"{timestamp} {event}\n")
...
def generate_bingo_cards(playernamelist, xsize, ysize):  # Funktion, die die Bingokarten generiert
    ...
    for k, player in enumerate(playernamelist):  # for Schleife, die die Bingokarten für jeden Spieler generiert
        ...
        log_event(k, f"Größe des Spielfelds: ({xsize}/{ysize})")  # Loggen der Spielfeldgröße
...
def mark_word(playernamelist, matrixlist):  # Funktion, die die Wörter markiert
    ...
    while True:  # Schleife, die die Wörter markiert
        ...
        for i, matrix in enumerate(matrixlist):  # Schleife, die die Bingokarten durchgeht
            for l, row in enumerate(matrix):  # Schleife, die die Zeilen der Bingokarte durchgeht
                if word_to_mark in row:  # Überprüfung, ob das Wort in der Zeile enthalten ist
                    j = row.index(word_to_mark)
                    if word_to_mark in marked_words:  # Überprüfung, ob das Wort bereits markiert ist
                        ...
                        log_event(i, f"{word_to_mark} ({j + 1}/{l + 1})")  # Loggen des markierten Worts und seiner Position
        ...
        for i, matrix in enumerate(matrixlist):  # Schleife, die die Bingokarten durchgeht
            if check_winner(matrix, marked_words):  # Überprüfung, ob ein Spieler gewonnen hat
                ...
                log_event(i, "Sieg")  # Loggen des Siegs
                log_event(i, "Ende des Spiels")  # Loggen des Spielendes
                log_files[i].close()  # Schließen der Log-Datei
                return  # beendet die Funktion
    for i in range(len(playernamelist)):  # Loggen des Spielendes für alle Spieler
        log_event(i, "Ende des Spiels")
        log_files[i].close()
...
def start_game():  # Funktion, die das Spiel startet
    ...
    for i in range(playercount):
        create_log_file(i)  # Erstellen der Log-Dateien für die Spieler
