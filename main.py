...
def sendfile(pipe, pids2, pipm1, xs, ys):
    for pa in range(pids2-1):
        
        with open(pipe[pa], 'w') as fifo:  # Öffnet die Pipes der Gegner, um Spieldaten zu schreiben
    ...
    try:
        while True:  # While-Schleife zur Erfassung des Befehls
            ...
            if JN == "newround":
                ...
                spieler_ids = []
                if playercount > 1:
                    spieler_ids = empfang(playercount, pid)  # Methode, welche die Liste der gegnerischen Pipes ermittelt und zurückgibt
                    spieler_ids.append(f"{pids}\n")  # Ergänzung der eigenen Pipe
                    sendfile(spieler_ids, playercount, file, xsize, ysize)  # Methode zur Sendung von Spielinformationen
